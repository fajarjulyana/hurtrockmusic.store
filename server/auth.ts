...
export function setupAuth(app: Express) {
  // Auto-generate SESSION_SECRET if not exists
  let sessionSecret = process.env.SESSION_SECRET;
  if (!sessionSecret) {
    sessionSecret = randomBytes(64).toString('hex');
    console.log('⚠️  SESSION_SECRET auto-generated. For production, set it in environment variables.');
  }
  ...
  app.put("/api/admin/users/:id", requireAdminKey, async (req, res) => {
    try {
      const { id } = req.params;
...
export function requireAdminKey(req: Request, res: Response, next: NextFunction) {
  ...
  // Check against environment variable or default admin key
  const validAdminKey = process.env.ADMIN_KEY || 'fajar';

  if (adminKey !== validAdminKey) {
    return res.status(403).json({ error: 'Invalid admin key' });
  }

  // Mark request as admin key authenticated
  (req as any).isAdminKeyAuth = true;
  next();
}
