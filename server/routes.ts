...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/payment/complete-order", async (req, res) => {
    try {
      ...
      console.log('Found cart items for manual completion:', cartItems.length);
      ...
      if (cartItems && cartItems.length > 0) {
        ...
        console.log('Creating manual order with data:', JSON.stringify(orderData, null, 2));
        ...
        console.log('No cart items found for manual completion');
        ...
        try {
          ...
          if (cartItems && cartItems.length > 0) {
            ...
            const orderData = {
              ...
              midtransOrderId: orderId,
              customerData: statusResponse.customer_details || {},
              paymentMethod: statusResponse.payment_type || 'unknown',
              createdAt: new Date(),
              updatedAt: new Date()
            };
