...
export default function ProductGrid({
  ...
  const products: Product[] = rawProducts.map(product => {
    ...
    const getFallbackImage = (categoryName: string) => {
      ...
      if (name.includes('guitar')) return guitarsImage;
      if (name.includes('vinyl') || name.includes('record')) return vinylImage;
      if (name.includes('drum')) return drumsImage;
      return guitarsImage; // default fallback
    };

    // Get category name from categoryName field or find it from categories
    let categoryDisplayName = product.categoryName;
    if (!categoryDisplayName && product.categoryId) {
      const category = categories.find(c => c.id === product.categoryId);
      categoryDisplayName = category?.name || 'Kategori Tidak Diketahui';
    }
    ...
    return {
      ...
      originalPrice: product.originalPrice ? 
        (typeof product.originalPrice === 'string' ? parseFloat(product.originalPrice) : product.originalPrice) : undefined,
      rating: parseFloat(product.rating || '0'),
      category: categoryDisplayName,
      image: product.imageUrl || getFallbackImage(categoryDisplayName),
