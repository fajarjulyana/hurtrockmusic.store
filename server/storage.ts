...
export class MemStorage implements IStorage {
  ...
  async addToCart(insertCartItem: InsertCartItem): Promise<CartItem> {
    ...
    const price = typeof insertCartItem.price === 'string' ? parseFloat(insertCartItem.price) : insertCartItem.price;
  ...
  async getCart(sessionId: string): Promise<(CartItem & { name: string; image: string; category: string })[]> {
    const result = Array.from(this.cartItems.values()).filter(item => item.sessionId === sessionId).map(item => {
      ...
      if (isNaN(itemPrice)) {
        ...
        const productPrice = product ? (typeof product.price === 'string' ? parseFloat(product.price) : product.price) : 0;
...
import { products, categories, cartItems, contactSubmissions, orders, orderItems, users, chatRooms, chatMessages, chatParticipants } from "@shared/schema";
