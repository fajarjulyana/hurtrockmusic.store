...
export class MemStorage implements IStorage {
  ...
  async createOrder(orderData: any, cartItems: CartItem[]): Promise<Order> {
    ...
    const order: Order = {
      id,
      customerName: orderData.customerData?.first_name || 'Unknown Customer',
      customerEmail: orderData.customerData?.email || 'unknown@example.com',
      customerPhone: orderData.customerData?.phone || null,
      shippingAddress: orderData.customerData?.address || 'Alamat tidak tersedia',
      fullShippingAddress: orderData.customerData?.address || null,
      orderStatus: orderData.orderStatus || 'processing',
      paymentStatus: orderData.paymentStatus || 'paid',
      total: orderData.totalAmount || 0,
      trackingNumber: null,
      midtransToken: orderData.midtransOrderId || null,
      processedBy: null,
      createdAt: now,
      updatedAt: now
    };
